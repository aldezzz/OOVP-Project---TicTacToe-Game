...
public class game extends javax.swing.JFrame {
    ...
    if (checkWinner(one, two, three, "X")) {
        Xwinner();
        
    }
    else if (checkWinner(four, five, six, "X")) {
        Xwinner();
    }
    else if (checkWinner(seven, eight, nine, "X")) {
        Xwinner();
    }
    else if (checkWinner(one, five, nine, "X")) {
        Xwinner();
    }
    else if (checkWinner(three, five, seven, "X")) {
        Xwinner();
    }
    else if (checkWinner(one, four, seven, "X")) {
        Xwinner();
    }
    else if (checkWinner(two, five, eight, "X")) {
        Xwinner();
    }
    else if (checkWinner(three, six, nine, "X")) {
        Xwinner();
    }
    else if (checkWinner(one, two, three, "O")) {
        Owinner();
    }
    else if (checkWinner(four, five, six, "O")) {
        Owinner();
    }
    else if (checkWinner(seven, eight, nine, "O")) {
        Owinner();
    }
    else if (checkWinner(one, five, nine, "O")) {
        Owinner();
    }
    else if (checkWinner(three, five, seven, "O")) {
        Owinner();
    }
    else if (checkWinner(one, four, seven, "O")) {
        Owinner();
    }
    else if (checkWinner(two, five, eight, "O")) {
        Owinner();
    }
    else if (checkWinner(three, six, nine, "O")) {
        Owinner();
    }
    else {
        draw();
    ...
    private boolean checkWinner(String a, String b, String c, String symbol) {
        return a.equals(symbol) && b.equals(symbol) && c.equals(symbol);
    }

    public void winner(String a, String b, String c) {
        if (checkWinner(a, b, c, "X")) {
           Xwinner();
        }
        else if (checkWinner(a, b, c, "O")) {
        Owinner();
        }
        else {
        // This shouldn't happen if the method is only called with valid arguments.
            throw new IllegalArgumentException("Invalid arguments for winner method.");
        }
    }

    public void winner(JButton b1, JButton b2, JButton b3) {
    String a = b1.getText();
    String b = b2.getText();
    String c = b3.getText();
    winner(a, b, c);
    ...
    private void btnexitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnexitActionPerformed
        ...
        if(JOptionPane.showConfirmDialog(frame,"You will be automatically logged out, do you still want to exit?", "Tic Tac Toe",
